...
/*
组合模式不但要管理层次结构，而且要执行菜单的操作，具有两个责任。组合模式
以单一责任设计原则换取透明性。
透明性: 通过让组件的接口同时包含一些管理子节点和叶节点的操作，客户就可以将组合
和叶节点一视同仁，也就是说，一个元素究竟是组合还是叶节点，对客户是透明的。
MenuComponent类中同时具有两种类型的操作，因为客户有机会对一个元素做出一些不恰当或者是
没有意义的操作(例如试图将菜单加入菜单项)，所以失去了一些"安全性".
我们可以采取另一种方式，将责任区分开来放在不同的接口中，这么一来，设计上就比较安全，但我们也
因此失去了透明性，客户的代码将必须用条件语句和instanceof操作符处理不同类型的结点。
*/

//现在我们设计自己迭代器来满足一些特定的需求，例如女招待可能想要游走整个菜单，调出素食项。
// 想要实现一个组合迭代器，让我们为每个组件都加上createIterator()方法，每个菜单和菜单项都必须实现这个方法。在MenuComponent中加入一个createIterator抽象方法
