...
* @Last Modified time: 2018-09-01 21:39:05
*/
...
/*
我们现在更改需求，加入赢家状态以及赢家状态类。见eclipse项目
我们看到，糖果机持有每个状态类的实例，机器的当前状态总是这些类实例之一。
糖果机调用接口中的动作方法，将动作委托给当前状态的类对象来处理，处理中也可能会改变糖果机的当前状态，因为每个状态对象中都有当前的
糖果机对象，在不同的方法中可以设置给糖果机想要的状态。
实现优点:
	将每个状态的行为局部化到它自己的类中
	对容易产生问题的if语句删除，以方便日后维护。
	让每一个状态"对修改关闭", 让糖果机"对扩展开放"，因为可以加入新的状态类

状态模式:
	允许对象在内部状态改变时改变它的行为，对象看起来好像改变了它的类
策略模式和状态模式有相同的类图，但他们的意图不同.
策略模式: 将可以互换的行为封装起来，然后使用委托的方法，来决定使用哪一个行为
*/
